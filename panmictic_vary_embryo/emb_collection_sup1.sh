...
do
    for ((j=1;j<2;j++)) #j traverse embryo_resistance_rate 每次都需改
    do
        for ((m=1;m<22;m++))  #  traverse drop_size 
        do
            k=$((($i+10)*10000+($j+10)*100+$m+10)) # cautious about file order 1 > 02 110000+11*100+11
            if (($k==111111));then
            python3 daisy_emb_sup_driver_0221.py -drop_size ${drop_size[$m]} -embryo_resistance_rate ${emb_res[$j]} -header > emb_data_sup/$k.part
            else
